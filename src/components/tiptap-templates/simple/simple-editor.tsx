...
// --- Lib ---
import { handleImageUpload, MAX_FILE_SIZE, UserCancelledImageUploadError } from "@/lib/tiptap-utils";
import type { EditorState } from "@tiptap/pm/state";
import { VideoEmbed } from "@/components/tiptap-node/video-embed-node/video-embed-extension";
import { parseTrustedVideoEmbed } from "@/lib/videoEmbedUrl";

function isCaretInCodeBlock(state: EditorState): boolean {
...
export type SimpleEditorProps = {
    ...
    /** 자유게시판 전용 — 영상 URL 붙여넣기 시 임베드 노드 허용 */
    ...
    const editor = useEditor(
        {
            ...
            editorProps: {
                ...
                handlePaste(view, event) {
                    ...
                    if (imageFiles.length === 0) {
                        if (videoPasteEnabled) {
                            const textPlain = data.getData("text/plain") ?? "";
                            const embed = parseTrustedVideoEmbed(textPlain);
                            if (embed && !isCaretInCodeBlock(view.state)) {
                                const nodeType = view.state.schema.nodes.videoEmbed;
                                if (nodeType) {
