...
function NaverMark({ className }: { className?: string }) {
    return (
        <svg className={className} viewBox="0 0 24 24" aria-hidden>
            ...
            <path fill="#03C75A" d="M7 7h2.55v4.35L14.45 7H17v10h-2.55v-4.4L9.55 17H7V7z" />
        </svg>
    );
}
...
export default function LoginModal({ open, onClose }: Props) {
    ...
    const onPwComplete = async (e: React.FormEvent) => {
        ...
        try {
            ...
            sonner.success(json.message ?? "비밀번호가 변경되었습니다. 로그인해 주세요.");
            setPwFlow("login");
            setPassword("");
        } catch (err) {
                        ...
                        <>
                            <div className="space-y-2">
                                ...
                                <div className="pt-1 text-xs text-gray-400">
                                    * 소셜 로그인 세션은 NextAuth가 관리합니다. 백엔드(Spring) API JWT 연동은 다음
                                    단계에서 토큰 교환 로직을 붙이면 됩니다.
                                </div>
                            </div>
                    ...
                    {pwFlow === "pw-request" && (
                        <form onSubmit={onPwRequest} className="space-y-3">
                            <p className="text-xs text-gray-400">
                                가입 시 등록한 이메일 주소로 6자리 인증코드를 보냅니다. 수신 메일함은
                                네이버·카카오·Gmail 등 어디든 가능합니다.
                            </p>
