...
export default function TopBar({ variant = "admin" }: TopBarProps) {
    ...
    const displayName = user?.nickname?.trim() || user?.username || session?.user?.name || session?.user?.email || null;
            ...
            <div className="mx-auto flex h-14 max-w-screen-2xl items-center justify-between px-4">
                <div className="text-sm font-semibold">
                    {variant === "site" ? (
                        <Link
                            href="/"
                            className="max-w-[min(100vw-10rem,36rem)] text-sm font-semibold leading-snug text-foreground hover:underline"
                        >
                            감빡이들의, 감빡이들에 의한, 감빡이들을 위한
                        </Link>
                    ) : (
                        "PRJT Admin"
                    )}
